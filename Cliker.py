...
class MainWindow(CTk):
    ...
    def show_menu(self):
        ...
        if self.menu_frame.winfo_width() >= 200 and self.is_show_menu:
            ...
            if self.label:
                self.label.destroy()
            getattr(self, "entry", None).destroy()
            getattr(self, "save_button", None).destroy()
        ...
        if msg_type == "TEXT":
            ...
            if len(parts) >= 4:
                ...
                try:
                    ...
                    self.add_message(f"Помилка вiдображення зображення: {e}")
            else:
